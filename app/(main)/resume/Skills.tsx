    ...
    <div className="not-prose grid grid-cols-[repeat(auto-fill,minmax(148px,1fr))] gap-3 text-sm">
      {skills.map(({ icon, name, url }) => {
        return (
          <a
            key={name}
            className="group relative flex aspect-video flex-col items-center justify-center gap-2 rounded-md border border-zinc-200/50 bg-zinc-100/50 text-white outline-none transition-all focusable hover:bg-zinc-200 dark:border-zinc-700/50 dark:bg-zinc-800/50 dark:hover:bg-zinc-800"
            href={url}
