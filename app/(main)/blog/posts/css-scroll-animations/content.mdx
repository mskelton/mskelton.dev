...
animations, let's first take a look at the kind of code you might think of for a
task like this.
...
scrolled so we can conditionally apply styles. Those styles might be a shadow
like shown above, or perhaps shrinking an image to a smaller size. Regardless of
the specifics, that's how it might look.

With scroll-driven animations, we can get a bit fancier. Try scrolling the
container below to see how the header background color slowly fades in as your
...
The HTML is once again very simple, just a container `div` that holds our header
...
The CSS is very similar to our first example containing some keyframes, making
our container scrollable, and some animation properties for our header.
...
instructs the browser the start and end scroll positions to use as the start and
end of the animation timeline. If not specified, this would be the start and end
of our scroll container, but by specifying a range of `0 80px`, the browser will
start the animation immediately after starting to scroll our container, and
finish when the user has scrolled 80px.

With the addition of `animation-range`, we also need to specify an
`animation-fill-mode` which is what we've done with the `forwards` keyword in
the `animation` property. This ensures that once we've scrolled past 80px in the
container, the element will retain the values from the last keyframe in the
animation, which in our case is the background color and blur effect. If we
don't specify this property, our element will reset back to it's initial values
before the animation took place which is not the intended behavior.
